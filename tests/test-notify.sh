...
# HUB_NOTIFY points at nothing on purpose: on a hub host the real /usr/local/bin/hub-notify
...
run() { HUB_NOTIFY="$W/no-hub-notify-here" STATE_DIR="$W/state" NOTIFY_DEDUP_SECONDS=0 HERMES_BIN="$W/bin/hermes" SEND_HOME="$W/home" ALERT_TARGET="${TARGET:-telegram}" LOG_FILE="$W/notify.log" bash "$HERE/templates/notify.sh" "$@" 2>"$W/err" >/dev/null; echo $?; }
...
[ "$(printf 'piped text' | HUB_NOTIFY="$W/no-hub-notify-here" STATE_DIR="$W/state" HERMES_BIN="$W/bin/hermes" SEND_HOME="$W/home" LOG_FILE="$W/notify.log" bash "$HERE/templates/notify.sh" 2>/dev/null >/dev/null; echo $?)" = "0" ] && grep -q "piped text" "$W/body" && ok "4 stdin works when no argument is given" || bad "4 stdin path broken"
...
# --- the way across to hub-notify (2026-09-17) --------------------------------
...
exec "$@"
EOF
chmod +x "$W/bin/hub-notify" "$W/bin/sudo"
export HN_ARGS="$W/hn-args" SUDO_LOG="$W/sudo.log"
hub() { : > "$SUDO_LOG"; rm -f "$HN_ARGS"; PATH="$W/bin:$PATH" NOTIFY_SENDER=hub-notify HUB_NOTIFY="$W/bin/hub-notify" LOG_FILE="$W/notify.log" STATE_DIR="$W/state" bash "$HERE/templates/notify.sh" "$@" 2>"$W/err" >/dev/null; echo $?; }
ran_through_sudo() { grep -q "^sudo -n $W/bin/hub-notify" "$SUDO_LOG"; }

if [ "$(id -u)" -eq 0 ]; then
  ok "11-16 skipped: root needs no way across, so there is nothing to choose (run as the service user)"
else
  [ "$(SUDO_RULE_RC=1 hub "needs a look")" = "0" ] && [ -f "$HN_ARGS" ] && ! ran_through_sudo \
    && ok "11 no rule for this binary: hub-notify is called directly, nothing hops" || bad "11 hopped through sudo without a rule" "$(cat "$SUDO_LOG")"
  [ "$(SUDO_RULE_RC=0 hub "needs a look")" = "0" ] && ran_through_sudo \
    && ok "12 a rule that already exists is used" || bad "12 an existing rule was ignored" "$(cat "$SUDO_LOG")"
  [ "$(SUDO_RULE_RC=0 NOTIFY_SUDO="" hub "needs a look")" = "0" ] && [ -f "$HN_ARGS" ] && [ ! -s "$SUDO_LOG" ] \
    && ok "13 NOTIFY_SUDO=\"\" never hops and never even asks sudo" || bad "13 sudo was touched although the host said never" "$(cat "$SUDO_LOG")"
  [ "$(SUDO_RULE_RC=1 NOTIFY_SUDO="sudo -n" hub "needs a look")" = "0" ] && ran_through_sudo && ! grep -q -- "-n -l" "$SUDO_LOG" \
    && ok "14 NOTIFY_SUDO=\"sudo -n\" always hops, without probing" || bad "14 the explicit hop was not taken" "$(cat "$SUDO_LOG")"
  grep -q -- "--lane card" "$HN_ARGS" && ok "15 the card lane still reaches hub-notify with its arguments" || bad "15 arguments lost" "$(cat "$HN_ARGS" 2>/dev/null)"
  NOTIFY_STATUS="repaired automatically" HN_ERR="hub-notify: cannot read /etc/hub/secret: Permission denied" SUDO_RULE_RC=1 hub "restarted it" >/dev/null
  grep -q "Permission denied" "$W/notify.log" \
    && ok "16 when the ledger is not reached, the log carries hub-notify's own words" || bad "16 the cause was swallowed" "$(tail -2 "$W/notify.log")"
fi
